#! usr/bin/python
# --- DOB: v1 - 25 Feb 2015
# --- Edit v2 - 30 Mar 2015: Function 11, 12 added
...
#----------------
def test_symmdyn(Dyn1):
   #
   #
   # number of free parameters in the passed array (in this case 7)
   Np = np.size(Dyn1)
...
# 
def test_inv_symmdyn(Dyn):
    #
    #  
    # Note that DynM has 48 elements and we will exract 7 
    # free parameters out of these.
    #
    para = np.zeros(7)
    para = np.array([Dyn[0] , Dyn[1] , Dyn[2] , Dyn[13] ,
                              Dyn[14], Dyn[17], Dyn[19]])
    #
    return para
    #
    #
...
def gee():
    #
    # these numbers are obtained from the symmdyn 
    Dyn0 = np.array([-1,-2,0])
    Dyn1 = np.array([1,0,0]) 
    Dyn2 = np.array([0,1,0])
    Dyn3 = np.array([-1,0,-2])
...
def genconfig(dyn):
    ...
    # 
    # generate parameters from 3 paras
    #
    dyn = symmdyn(dyn)
    ...
    pos = pos.reshape((Nc,nat,ndim))
    #
    # write them to the scf.in files
    mirror ='cp scf.in ./InFiles/scf'
    # string of atoms
    atmtyp = RJ.atomfind(fnam0)
    #
    for i in range(0,Nc):
        # 
        outs = mirror + str(i) + ".in"        
        # for copying scf.in as scfi.in
        os.system(outs)
        nfile = " ./InFiles/scf" + str(i) + ".in"
        #
        temp = "s/ line0/" + str(i) + "' ./" 
        comd = 'sed -r -i \"' + temp + '"' + nfile
        os.system(comd) 
        for ii in range(0,nat):
             replc = ( "s/line" + str(ii+1)+ "/ " + atmtyp[ii] +
                        str(pos[i][ii][:]).strip('[]') + '/\"' )
             cmd = 'sed -r -i \"' + replc + nfile
             os.system(cmd)
    #
    #
    return
